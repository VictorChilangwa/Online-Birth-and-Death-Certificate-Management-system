...
$id = (int)$_GET['id'];
$result = $conn->query("SELECT b.*, u.fullname as staff_name, u.hospital_name FROM birth_applications b JOIN users u ON b.registered_by = u.id WHERE b.id=$id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update = "UPDATE birth_applications SET status='$status', admin_remarks='$remarks' WHERE id=$id";
...
<body>
    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            <h1 style="color: #2c3e50; margin-bottom: 0.5rem;">Review Birth Registration</h1>
            <p style="color: #7f8c8d; margin-bottom: 2rem;">Assess document and details validity to approve or reject registration.</p>

            <div class="detail-card">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 3rem;">
                    <div>
                        <h3><i class="fas fa-child"></i> Subject Details</h3>
                        <p><strong>Child Name:</strong> <?php echo $row['child_fullname']; ?></p>
                        <p><strong>Gender:</strong> <?php echo ucfirst($row['gender']); ?></p>
                        <p><strong>DOB:</strong> <?php echo $row['date_of_birth']; ?></p>
                        <p><strong>Place:</strong> <?php echo $row['place_of_birth']; ?></p>
                        <p><strong>Father:</strong> <?php echo $row['father_fullname'] ?: 'N/A'; ?></p>
                        <p><strong>Mother:</strong> <?php echo $row['mother_fullname']; ?></p>
                    ...
                    <div>
                        ...
                        <?php if($row['status'] === 'approved'): ?>
                            <p style="margin-top: 1rem; margin-bottom: 1.5rem;">
                                <a href="../staff/view_certificate.php?type=birth&id=<?php echo $row['id']; ?>" class="btn" style="background:#3498db; text-decoration:none; display:inline-flex; align-items:center; gap:0.5rem; width: auto;"><i class="fas fa-certificate"></i> View Official Certificate</a>
                            </p>
                        <?php endif; ?>
